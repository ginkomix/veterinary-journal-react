...
class Table extends React.Component {
  ...
  getFilterItems() {
    ...
    if (this.props.filter.text !== "") {
      arr = arr.filter(
        item =>
          item.title.toUpperCase().includes(this.props.filter.text) ||
          item.description.toUpperCase().includes(this.props.filter.text)
      );
    }

    if (this.props.filter.dataMax) {
      arr = arr.filter(
        item => Date.parse(this.props.filter.dataMax) > Date.parse(item.date)
      );
    }
    if (this.props.filter.dataMin) {
      arr = arr.filter(
        item => Date.parse(this.props.filter.dataMin) < Date.parse(item.date)
      );
    }
    return arr;
  }
    ...
    return (
      <div className="main-table">
        {sort.sortBy(this.getFilterItems(), this.props.sort).map(item => {
          return (
            <div
              onClick={this.returnIdItem}
              id={`item${item.id}`}
              className={`${item.id} ${
                this.props.id === item.id ? "active" : ""
              } block-table `}
              key={item.id}
            >
              <div className={`${item.id} inf1 `}>
                <input
                  className={item.id}
                  type="checkbox"
                  checked={item.done}
